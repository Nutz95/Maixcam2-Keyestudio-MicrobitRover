...
from lib.ball_follow_hud import BallFollowHud
from lib.gyro_calib_overlay import GyroCalibOverlay
...
class UiDrawer:
  ...
  def __init__(self, width, height):
    ...
    self._gyro_calib = GyroCalibOverlay(
      width, height, self._back_pad, self._img_back,
    )
  ...
  def _layout_buttons(self):
    ...
    # Single GYRO hit target (top-right); works before Xbox connect.
    gyro_w = 96
    self._gyro_rect = [self.width - pad - gyro_w, pad, gyro_w, back_size]
    btn_h = 72
    gap = 12
    ...
    # Full-width disconnect when connected (GYRO lives top-right only).
    self._disconnect_rect = [gap, y, self.width - gap * 2, btn_h]
  ...
  def draw_overlay(
    ...
    if imu_snapshot is not None and imu_snapshot.calibrating:
      self._gyro_calib.draw(img, imu_snapshot)
      return

    self._draw_gyro_button(img, imu_snapshot)

    if connected:
      self._draw_bottom_bar(img)
      self._draw_button(
        img, self.disconnect_rect(), "DISCONNECT", image.Color.from_rgb(180, 60, 40),
      )
  ...
  def _draw_gyro_button(self, img, imu_snapshot):
    ...
    label = "HOLD" if imu_snapshot is not None and imu_snapshot.calibrating else "GYRO"
    self._draw_button(img, self.gyro_rect(), label, color)
  ...
  def _draw_progress(self, img, status, progress):
    ...
    img.draw_string(
      (self.width - size.width()) // 2, bar_y - 28, label, image.COLOR_WHITE, scale=1.1,
    )
  ...
  def _draw_dpad(self, img, cx, cy, dx, dy):
    ...
    for ox, oy, active in dirs:
      img.draw_circle(cx + ox, cy + oy, r, on if active else dim, thickness=-1)
